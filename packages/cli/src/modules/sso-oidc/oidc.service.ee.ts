...
export class OidcService {
	...
	async loginUser(callbackUrl: URL, storedState: string, storedNonce: string): Promise<User> {
		...
		try {
			...
			this.logger.info(
				`Successfully exchanged authorization code for tokens ${JSON.stringify(tokens)}`,
			);
		} catch (error) {
	...
	private async applySsoProvisioning(user: User, claims: Record<string, unknown>) {
		...
		this.logger.info(
			`Successfully applied SSO provisioning for user ${user.id} with claims ${JSON.stringify(claims)}`,
		);
	}
